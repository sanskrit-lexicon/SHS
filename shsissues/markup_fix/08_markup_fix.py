"""
Markup fixer + audit for shs.txt (SHS).
Counterpart of PWG#175, PWK#113, and siblings.

SHS-specific notes: No whitespace hits. Essentially clean.

Inputs: ../../../csl-orig/v02/shs/shs.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "shs" / "shs.txt", HERE / "shs.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "shs_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(Poem|s)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for shs.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"SHS markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against shs.txt.\n")
        f.write(f"Paired tags: <Poem>, <s>\n")
        f.write(f"Notes: No whitespace hits. Essentially clean.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <Poem>, <s>\n\n")
